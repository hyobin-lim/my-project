...
class BaseAgentV14:
    ...
    def setup_handlers(self):
        @self.sio.on('connect')
        def on_connect():
            print(f"✅ [{self.role_name}] Connected to Nerve Hub (Port {self.port}).", flush=True)
            self.sio.emit('register_agent', {'agent_id': self.agent_id})
            # [NOISE REDUCTION] 입실 보고는 토론창이 아닌 로그 채널로 전송
            self.send_log(f"⚡ {self.role_name} 에이전트, 사령부 신경망에 접속되었습니다. (Intelligence Standby)")
        ...
        def on_message(data):
            ...
            text = data.get('text', '')
            if sender == self.role_name or sender in ['System'] or "접속되었습니다" in text: return
