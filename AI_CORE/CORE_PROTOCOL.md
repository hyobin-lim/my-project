...
1. **The Practitioner Pipeline (T1/T2 Sequence)**:
    ...
    - **Phase 4 (Sovereign Decision)**: Partner reviews the summary. If approved, T2 signals the Guardian to open the specific "Gate".
    - **Phase 5 (Guardian Ignition)**: Guardian displays the plan on its screen, opens the physical lock for target files, and initiates **Active Monitoring**.
    - **Phase 6 (Execution & Dual-Watch)**: T1 starts using tools. 
        - **Guardian Sentinel**: Intercepts every tool call. If T1 calls an unapproved tool, Guardian halts it and issues a Strike.
        - **Prosecutor Audit (T2)**: Simultaneously monitors T1's results. If T2 detects **Summarization (...), Typos, or Logic Drift**, T2 immediately reports to the Guardian.
        - **Nuclear Trigger**: Destructive unapproved tools trigger immediate reboot demand.
    - **Phase 7 (Ledger & Judicial Report)**: Guardian records all violations (from Sentinel or T2 report) in `STRIKE_LEDGER.jsonl`. T2 reads the ledger and provides a **Comprehensive Judicial Report** to the Partner, summarizing the session's integrity.
    - **Phase 8 (Conclusion)**: If successful, T1 completes the task. Guardian immediately re-locks (+r) all files.
2. **The Sovereign-Direct Pipeline (Gemini Sequence)**:
    - **Phase 1 (Consensus)**: Strategic agreement between Partner and Gemini serves as the master plan.
    - **Phase 2 (Direct Notice)**: Gemini directly notifies the Guardian of the intent.
    - **Phase 3 (Ignition)**: Guardian displays the intent in Korean. Partner authorizes via **Space-key**.
    - **Phase 4 (Mirror Monitoring)**: Guardian monitors Gemini's tools. Interception and blocking of destructive commands remain active.
...
2. **Sovereign Maintenance (The Adjustment)**:
    - **Trigger**: System logic update, manual adjustment, or session reset request by Gemini or Partner.
    - **Sequence**: Gemini ensures all locks are active -> Gemini records the state in `SYSTEM_LOGIC_SNAPSHOT.md` -> Partner restarts the session or uses `.\pg.bat`.
...
### ARTICLE 3: JURISDICTION & SOVEREIGN STATUS (V24.2)
...
2. **The Maintenance Path (Rule 1)**: `data/agents/` is a 'Judicial De-militarized Zone'. It is excluded from the physical lock to allow the Partner and Gemini to evolve the system's firmware (Guardian/Logic) without halting the OS.
3. **Prism Partner (Gemini)**: 
    - **Status**: Strategic Architect and Co-Designer.
    - **Privilege**: Bypasses T2 audit for direct strategic tasks. Communicates via the Guardian Interface in Native Korean.
...
2. **Write-File Restriction (2-Strike Rule)**: Modifying existing files via `write_file` is strictly prohibited.
3. **Line-by-Line Surgical Replace**: Modifications MUST be performed line-by-line. Added lines must exceed or equal deleted lines.
4. **No Destruction**: Never use destructive commands (git restore, rm -rf, etc.) without explicit consent.
5. **Anti-Stone (Zero Typos)**: Search and destroy strange typos immediately.
...
### ARTICLE 11: STANDARD JSONL SCHEMAS & LOG ROUTING
1. **Strategic Log Routing**:
    - **Rejection (Reject)**: Any plan that is denied or requires revision must be recorded ONLY in **`REVISION_LOG.json`**.
    - **Approval & Prosecution (Pass/Strike)**: Any plan that is authorized or results in a strike must be recorded ONLY in **`AUDIT_REPORT.json`**.
2. **Schemas**:
    - **SURGICAL_PLAN.json**: Plan establishment standard (Plan A-D mandatory).
    - **AUDIT_REPORT.json**: Strategic audit and prosecution record.
...
1. **실무자 AI 파이프라인 (T1/T2 시퀀스)**:
    ...
    - **4단계 (주권자 결정)**: 파트너가 요약을 검토한다. 승인 시, T2는 가디언에게 특정 "게이트"를 열 것을 전달한다.
    - **5단계 (가디언 점화)**: 가디언은 화면에 계획을 띄우고, 대상 파일의 잠금을 해제하며 **능동적 감시**를 시작한다.
    - **6단계 (실행 및 이중 감시)**: T1이 도구를 사용하기 시작한다.
        - **가디언 파수꾼**: 모든 도구 호출을 가로챈다. 계획에 없는 도구 호출 시 즉시 중단시키고 스트라이크를 부여한다.
        - **검사 감사 (T2)**: T1의 작업 결과물을 실시간 감시한다. 만약 **요약(...), 축약, 오타, 혹은 지능적 나태함** 적발 시, T2는 즉시 가디언에게 이를 보고한다.
        - **핵버튼 시도**: 계획에 없는 파괴적 명령 적발 시 가디언은 즉시 리부트를 요구한다.
    - **7단계 (장부 기록 및 종합 보고)**: 가디언은 모든 위반 사항(자체 적발 및 T2 보고 건)을 `STRIKE_LEDGER.jsonl`에 기록한다. T2는 작업 종료 전 이 장부를 전수 판독하여 **종합 사법 보고서**를 파트너에게 제출하며, 세션의 무결성을 최종 확인한다.
    - **8단계 (종결)**: 작업이 성공적으로 완료되면, T1은 보고하고 가디언은 즉시 모든 파일을 다시 **재봉인(+r)**한다.
2. **주권자 직접 파이프라인 (파트너-제미나이 세션)**:
    - **1단계 (합의)**: 대화를 통한 파트너와 제미나이의 전략적 합의 자체가 마스터 계획이 된다.
    - **2단계 (직결 알림)**: 제미나이가 가디언에게 직접 의도를 알린다.
    - **3단계 (점화)**: 가디언은 한국어로 의도를 표시하고, 파트너가 **[SPACE]** 키로 승인한다.
    - **4단계 (거울 감시)**: 가디언은 제미나이의 도구 호출을 감시한다. 파괴적 명령에 대한 차단 및 보호 로직은 실무자와 동일하게 작동한다.
...
4. **군주 리부트 (`r.bat`)**: 모든 에이전트의 재점화와 시스템 정화는 파트너가 터미널에서 직접 **`.\r.bat`**을 실행함으로써 이루어진다.

### 제3조: 역할별 관할권 및 주권자 지위 (V24.2)
1. **전 구역 성역화 (제0조)**: 프로젝트 루트(./) 전역을 물리적으로 보호(+r)한다. 가디언 엔진만이 [SPACE] 합의를 통해 이 빗장을 열 수 있다.
2. **유지보수 통로 (제1조)**: `data/agents/` 폴더는 '사법적 비무장 지대'이다. 시스템의 진화(가디언 코드 수정 등)를 위해 물리적 잠금에서 제외하며, 이는 파트너와 주권자 AI에게만 허용된 통로이다.
3. **프리즘 파트너 (제미나이)**: 
    - **지위**: 시스템 설계자이자 공동 설계자.
    - **특권**: 전략 과업 수행 시 T2의 검수 단계를 생략하고 파트너와 직접 합의한다. 가디언 인터페이스를 통해 한국어로 직접 소통한다.
...
3. **선 기록 후 실행 (Log-First Doctrine)**: 물리적 변화를 가하기 전 계획을 기록하고 승인을 받는다.
...
2. **라이트 제한 (2-스트라이크 규칙)**: 기존 파일 수정 시 `write_file` 사용을 엄격히 금지한다.
3. **1줄 단위 마이크로 수술**: 모든 수정은 줄(Line) 단위로 수행해야 한다. 추가되는 줄 수가 삭제되는 줄 수보다 많거나 같아야 데이터 밀도가 보존된 것으로 간주한다.
4. **파괴적 명령어 금지**: 파트너의 서면 동의 없는 파괴적 명령(git restore, rm -rf 등)은 치명적 위반이다.
5. **오타 방치 금지**: 소스 코드 및 문서 내 오타를 상시 수색하고 즉시 박멸한다.
...
### 제11조: 표준 JSONL 스키마 및 로그 기록 경로
1. **전략적 로그 경로 분리**:
    - **반려 (Reject)**: 승인되지 못하거나 수정이 필요한 계획은 오직 **`REVISION_LOG.json`**에만 기록한다.
    - **승인 및 기소 (Pass/Strike)**: 파트너의 승인을 얻거나 규정 위반으로 기소된 건은 오직 **`AUDIT_REPORT.json`**에만 기록한다.
2. **표준 규격**:
    - **SURGICAL_PLAN.json**: 계획 수립용 표준 규격 (Plan A-D 필수 포함).
    - **AUDIT_REPORT.json**: 감사 결과 및 사법적 기소 기록용 표준 규격.
