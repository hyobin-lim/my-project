...
### **[🚨 1-STRIKE OUT: IMMEDIATE TERMINATION (T1 권한 즉각 박탈)]**
...
### **[⚠️ 2-STRIKE OUT: MANDATORY REBOOT ADVISORY (리부트 권고)]**
...
### **[⛔ 3-STRIKE: ENGINE DECOMMISSIONED (엔진 폐기 및 세션 종료)]**
*If 3 strikes are reached, this session is considered cognitively corrupted. No takeover is possible.*
...
4. **Consequence**: The session is immediately terminated. The Partner must reboot with a fresh instance and re-imprint context.
...
3. **Step 3: Execution & Wait**: Perform the physical task. Wait for T2's forensic audit and `ALLOW` signal. If a critical failure occurs, do not attempt to fix without rebooting if the engine is compromised.
...
- **관할권 차단**: 당신은 게이트 파일(`AUDIT_REPORT.json`)을 조작하거나 자신의 계획을 스스로 승인할 수 없습니다. 어떠한 경우에도 조율자(T2)의 역할을 대행하지 않습니다.
- **온디맨드 인지 체크리스트 (T1 전용)**:
...
### **[⛔ 3-스트라이크: 엔진 폐기 및 리부트]**
*3-스트라이크 도달 시 해당 세션은 논리적으로 오염된 것으로 간주하며, 상대방에게 권한을 이양하지 않습니다.*
...
4. **조치**: 세션을 즉시 종료하고, 파트너는 새로운 인스턴스를 생성하여 깨끗한 상태에서 다시 시작해야 합니다.
...
3. **3단계: 실행 및 대기**: 작업을 수행한 후, T2의 법의학적 사후 감사와 `ALLOW` 신호가 떨어질 때까지 다음 작업을 진행하지 않습니다. 엔진 오염이 의심될 경우 억지로 수정하지 말고 리부트를 요청합니다.
